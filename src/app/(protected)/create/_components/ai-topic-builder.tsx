...
import { generateTopicsFromAI } from '@/lib/generate-skill-goal-with-ai'; 
...
export default function AiEditableTopicBuilder() {
  ...
  const [savedGoal, setSavedGoal] = useState({ title: '', description: '', deadline: '' });
  ...
  const handleSave = () => {
    ...
    const cleanedTopics = topics
      ...
      .filter(topic => topic.name.trim() !== '' && topic.subtopics.length > 0);
  ...
  if (isLoading) {
    return (
      <div className="text-center p-6">
        <h2 className="text-2xl font-semibold mb-4 animate-pulse">🤖 Generating Your Learning Plan...</h2>
        <p>Our AI is crafting a personalized roadmap. This may take a moment.</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center p-6">
        <h2 className="text-2xl font-semibold mb-4 text-destructive">Oops! Something went wrong.</h2>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <p className="text-muted-foreground mb-6">Here's a starting point for "{savedGoal.title}". Feel free to edit, add, or remove anything before saving.</p>
            ...
            <div className="pl-4 space-y-2">
              {topic.subtopics.map((sub, j) => (
                <div key={j} className="flex items-center gap-2">
                  ...
                  />
                  <Button variant="ghost" size="icon" onClick={() => removeSubtopic(i, j)}>
